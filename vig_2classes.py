...
class Decoder(nn.Module):
    def __init__(self, in_dim=768, out_dim=10, act='relu'):
        ...
        self.regression_head=nn.Sequential(
            ...
            nn.ConvTranspose2d(in_dim//8,2,kernel_size=2,stride=2),
        ...
        self.classification_head=nn.Sequential(
            ...
            nn.ConvTranspose2d(in_dim//8,out_dim,kernel_size=2,stride=2),
...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        self.output_decoder=Seq(
            nn.Conv2d(self.n_blocks*channels,self.n_blocks*channels//4,1),
            nn.BatchNorm2d(self.n_blocks*channels//4),
            act_layer(act),
            # nn.Conv2d(self.n_blocks*channels//4,channels,1),
            # nn.BatchNorm2d(channels),
            nn.Conv2d(self.n_blocks*channels//4,self.n_blocks*channels//8,1),
            nn.BatchNorm2d(self.n_blocks*channels//8),
            act_layer(act),
            nn.Conv2d(self.n_blocks*channels//8,channels,1),
            nn.BatchNorm2d(channels),
            act_layer(act),
            #2022.5.22 new 为了进一步降低维度 PS:是否再加个3*3卷积更好？（但是信息会不会相互交换了）
...
def vig_ti_224_gelu(pretrained=False, **kwargs):
    class OptInit:
        def __init__(self, num_classes=2, drop_path_rate=0.0, drop_rate=0.0, num_knn=9, **kwargs):
            ...
            self.bias = True # bias of conv layer True or False     #paper上不是false？？？
